...
set @in_ltpid = if(trim(in_ltpid)='','p_toffdoc_duty_save',in_ltpid);

# 將toffdoc 結轉成 toffdoc_duty
/*

call p_toffdoc_duty_save('microjet','a00514','ltpid',16419);
16419= tOffDoc.rwid
*/
...
left join tofftype c on c.offtypeguid=
 (select offtypeguid from toffdoc x where x.rwid=in_RWID) ;
...
select @in_ltUser,@in_ltpid,a.OffDocGuid,b.dutydate,a.OffTypeGuid,b.OffMins
from toffdoc a
left join tmp04 b on 1=1
where a.rwid=@in_RWID;
