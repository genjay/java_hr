...
if err_code=0 Then # 10 抓 depGuid
   set @depguid='';
   Select codeGuid into @depguid From tcatcode Where Syscode='A07' And OUguid=@in_OUguid And CodeID=@in_DepID;
   if ifnull(@depguid,'')='' then set err_code=1; set @outMsg="部門代號錯誤"; end if;
end if;
...
if err_code=0 then # 20 jei workGuid
   ...
   if ifnull(@workguid,'')='' then set err_code=1; set @outMsg=concat(in_Dutydate,"班別代號錯誤"); end if;
end if;
...
if err_code=0 Then # End
   ...
   values (@depguid,@in_dutydate,@in_holiday,@workguid)
