drop procedure if exists p_AccountChange;

delimiter $$

create procedure p_AccountChange
(
...
if err_code=0 then # 10 判斷新舊密碼是否一樣
  if in_OldPassWd=in_NewPassWd Then set err_code=1; set outMsg='新舊密碼不可一樣'; else set outMsg='Pass-新舊密碼不同';end if;
end if;
...
if err_code=0 Then # 90 修改宓碼
  Update tAccount set PassWD=Sha(in_NewPassWd) 
  Where Aid_Guid=in_Aid_Guid;
  set outMsg='密碼修改完成';
end if;
