...
drop table if exists tProc_dtime;
create temporary table tProc_dtime engine=memory
select ouguid,cardno,dtcardtime 
from tcardtime
where ouguid=@var_ouguid
and dtcardtime between yestoday and nextday;
alter table tProc_dtime 
 add index i01 (ouguid,cardno) using btree,
 add index i02 (dtcardtime) using btree;
...
left join tProc_dtime c on a.ouguid=c.OUguid and a.cardno=c.cardno
